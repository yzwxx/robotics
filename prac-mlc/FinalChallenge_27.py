...
def rotateSonar(threshold,currentParticles):
    ...
    while not interface.motorAngleReferencesReached([motors[2]]):
        ...
        angle = reading_angle[0][0]-initial_motor[0][0]
        distance = interface.getSensorValue(port2)[0]
    ...
    if not resultDistance:
        ...
        for i in range(len(resultAngle)):            
            ...
            new_angle = curr_angle + resultAngle[i] - math.pi/2 # i changed here
            new_x = curr_x + objDistance*math.cos(new_angle)
            new_y = curr_y + objDistance*math.sin(new_angle)
            
            distToWallThreshold = 10
            if notWall(new_x,new_y,distToWallThreshold):
                resultAngle_object.append(resultAngle[i])
                resultDis_object.append(resultDistance[i])
        if not resultDis_object:
            minDistance = None
            angle = None
...
def notWall(x,y,distToWallThreshold):
    judge = 1  # 1 means not wall
    for wall_index in range(numOfWall):
        #print wall_index
        Ax = mymap.walls[wall_index][0]
        Ay = mymap.walls[wall_index][1]
...
sectorB = [(126,42+5*0),(126-5*0,74),(126-5*0,106), (126-5*0,136), (126-5*0, 168), (126-5*0,106)]
sectorC = [(42-5*0,106), (42-5*0, 136)]
sectorD = [(84, 30)]
...
for k in range(0, 4):
    ...
    for Wx,Wy in wayPoint:
        ...
        if objDistance != None and objAngle != None:
            ...
            new_angle = curr_angle+ objAngle
            new_x = curr_x +objDistance*math.cos(new_angle)
            new_y = curr_y +objDistance*math.sin(new_angle)
            print 'nextPosition:'+ str(new_x) + ','+str(new_y)+ ','+str(new_angle)
                      
                        
            distToGo, angleToGo, dest_angle =  navigateToWaypointAux(new_x, new_y, origin)
                
    
            #if bumpDetection (to fill in if decide to use touch sensor)
