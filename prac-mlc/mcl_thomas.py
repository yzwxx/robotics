...
def expected_depth(Ax, Ay, Bx, By, x, y, theta):
    m= ((By-Ay)*(Ax-x) - (Bx-Ax)*(Ay-y)) / ((By-Ay)*math.cos(theta)-(Bx-Ax)*math.sin(theta)+0.000001);
    return m              
...
def calculate_likelihood(x, y, theta, z, sigma_s, K):
    ...
    for wall in map.walls:
        ...
        Bx = wall[2]
        By = wall[3]
        m = expected_depth(Ax, Ay, Bx, By, x, y, theta)
        x0, y0 = sonar_coordinate(x,y,theta, m)
        
        print(Ax, Ay, Bx, By, x0, y0, m)
        
        if (check_sonar_coordinate(Ax, Ay, Bx, By, x0, y0) and m >0):
            wall_dist [m] = wall
...
test =calculate_likelihood(168, 1.0, 0.0, 42.0, 0.1, 0.0000)  
